...
void parse_csv(FILE* file, long int* matrix, int n_row, int n_col)
{
    for(int i = 0; i < n_row; i++)
    {
        ...
        {
            fscanf(file, "%ld,", &matrix[i * n_col + j]); //Reads the matrix file & inserts into it's allocated space
        }
...
int main(int argc, char* argv[])
{
    ...
    */
    ...
    {
        long int local_max = 0; //Each thread will have it's own local max and second max that it will update as it calculates the values in the result matrix
        long int local_second_max = 0;
        ...
        for(int i = 0; i < n_row1; i++)
        {
            for(int j = 0; j < n_col2; j++)
            {
                ...
                else if(current > local_second_max && current != local_max)
                {
                    local_second_max = current;
                }
            }
        }

        //Updating the global values - Needs to be critical so that we do not have any race conditions.
        #pragma omp critical
        {
            if (local_max > global_max) 
            {
                // The old global_max and the local_second are now candidates for global_second.
                long int temp_old_max = global_max;
                global_max = local_max;
                
                // Second max is the higher
                if (temp_old_max > local_second_max)
                {
                    global_second_max = temp_old_max;
                }
                else
                {
                    global_second_max = local_second_max;
                } 
            } 
            else if (local_max > global_second_max && local_max != global_max) 
            {
                global_second_max = local_max;
            }
            
            // Also check if the local_second beats our current global_second
            if (local_second_max > global_second_max && local_second_max != global_max) 
            {
                global_second_max = local_second_max;
            }
        }
    ...
    fprintf(outputFile, "%ld", global_second_max);
