...
void process_tetranucs(struct Genes genes, int* gene_TF, int gene_index) 
{
    ...
    {
        // Get the substring from i to i+3 in the DNA sequence
        char c1 = genes.gene_sequences[gene_index * GENE_SIZE + i];
        char c2 = genes.gene_sequences[gene_index * GENE_SIZE + i + 1];
        char c3 = genes.gene_sequences[gene_index * GENE_SIZE + i + 2];
        char c4 = genes.gene_sequences[gene_index * GENE_SIZE + i + 3];
...
double find_median(int* freqs, int num_genes) 
{
    ...
    // Apparently mergesort is best for parallelization, so I'm implementing that
    merge_sort(freqs, 0, num_genes - 1);

    // TODO: median if even
    if (num_genes % 2 == 0)
        return (freqs[num_genes / 2 - 1] + freqs[num_genes / 2]) / 2.0;
